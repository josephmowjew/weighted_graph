...
# Draw the graph
pos = nx.spring_layout(weighted_graph.graph)  # Layout algorithm (you can choose other layouts)
nx.draw(weighted_graph.graph, pos, with_labels=True, node_size=700, node_color="skyblue", font_size=8, font_color="black")

# Highlight the shortest path
if result:
    path = result[0]  # Retrieve the path from the result
    edge_labels = {(path[i], path[i+1]): weighted_graph.graph[path[i]][path[i+1]]['weight'] for i in range(len(path)-1)}
    nx.draw_networkx_edge_labels(weighted_graph.graph, pos, edge_labels=edge_labels, font_color='red')
